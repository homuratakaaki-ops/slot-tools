...
## 88. S12 貯玉引出のパーソナル判定修正とリザルト表示の整理
...
- 再発防止として、`usesTapInvestmentMode(session) && Boolean(store?.isPersonal)` の判定を `usesPersonalTapMode(session, store)` の1関数にまとめ、投資玉数そのものも `playerInvestedBalls(session, totals, store)` に集約した。`profitYenForSession()` と `withdrawBalls` は同じ関数を通す。条件式を他所へ書き戻さないことは、`tests/yutime-v3.test.js` で「この条件式はファイル全体で1箇所だけ」というアサートで固定した。
- 定義は `貯玉引出 = プレイヤーが自分の玉から出した投資合計（現金は含めない）`。パーソナル店のtapModeは `開始持ち玉 + 再プレ投資玉`（S4/§5で再プレを足した扱いは維持）、それ以外は `持ち玉投入 + 再プレ投資玉`（B81由来のタップ投資合計）。`貯玉預入`（終了玉＋残保留）と表裏になる。
- 検算: 非パーソナル店・125玉×19回 → **2,375個**（従来は開始持ち玉の4,625個）。パーソナル店・開始持ち玉2,500＋再プレイ500 → **3,000個（不変）**。tapModeでない旧データは、店の別なく `持ち玉投入＋再プレ` になる（実収支の非パーソナル式と同じ扱い。回帰テストの合成データで 125個 → 375個）。
