...
- schema 変更はない。パーソナル運用の通常回転率では、`hitRemainBalls` が入力されている当選済みセッションに限り、通常時投資玉から当選時残り玉を控除した値を分母にする。未入力時は従来どおり通常時投資玉をそのまま使う。
...
- tapMode の実収支は `(endTotalBalls + zanhoryuBalls - startMochidama - saipurei投資玉) * 交換単価 - 現金投資額面` とし、非パーソナルはB81の財布実額式を維持する。差玉の計算は従来どおり台上すべて基準のまま変更しない。
- 転記用の `貯玉引出` はtapModeでは開始時持ち玉を表示する。容量予算の増分はなし。
