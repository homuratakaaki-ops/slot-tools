...
- `ramClear` は当日だけ有効。保存値は `cleared`（ラムクリされてた）/ `not_cleared`（ラムクリされてなかった）/ null（未確認）の3値で、ラムクリ傾向の生データとして残す。翌日のマップ表示は未確認に戻るが、日付付き記録自体は残る。
...
| `ytv3:premigrate` | 将来のスキーマ変更時に、旧 schema の raw データを退避する | 読み込み時、保存済み `version` が実装中の `SCHEMA_VERSION` と異なり、かつ `ytv3:premigrate` が未作成の場合。B1 では schema 1 から 2、B2 では schema 2 から 3、B3 では schema 3 から 4、B4 では schema 4 から 5、B5 では schema 5 から 6、Phase 4差し戻し1では schema 10 から 11、B10では schema 11 から 12、B20では schema 12 から 13、B13/B22では schema 13 から 14、B23では schema 14 から 15、B25では schema 15 から 16 への更新時に作成対象となる | 変更前の `ytv3:data` raw 文字列 |
...
- ラムクリ傾向分析: `dailyState` に蓄積した `ramClear: "cleared" / "not_cleared"` と閉店チェック回転数から、前日ヤメ回転数帯ごとのリセット率を見る（否定確認も含む）。
...
- 旧 `ramClear` 値は読み込み時に後方互換変換する。`retained` は `prevInvalid: false`、`cleared` / `unknown` / `hitToday` は `prevInvalid: true` として扱う。`cleared` のみ `ramClear: "cleared"` も残す。
- 台詳細の4状態ボタンと「前回ヤメ（履歴）」カードは撤去する。台詳細は期待値判定、前日ヤメ手入力、台メモ、参考回転率などの統計に専念する。
- マップの旧状態バッジ「リ」「不」「当」「据」は撤去する。B25以降、ラムクリされてた記録がある当日は「ラ」マーク、ラムクリされてなかった記録がある当日は緑系の「据✓」マークを表示する。
- マップ上部に「朝チェック」トグルを置く。ON中に台番をタップすると、前日分無効チェックとラムクリ確認の排他トグル「されてた」「されてなかった」を持つミニ選択を開く。「されてた」をONにすると前日分無効も自動ONにする。「されてなかった」は前日分無効に触れず、据え置き確認として宵越しを有効のまま扱う。
- 朝チェックは閉店チェックと同じく、マップ定義の列内順序で自動遷移する。除外台は飛ばし、対面列や別マップへは渡らない。
...
- `dailyState[machineId][date].ramClear` は `cleared`（されてた）/ `not_cleared`（されてなかった）/ null（未確認）の3値にする。旧 `cleared` / null はそのまま後方互換で読む。
- 朝チェックの「されてた」は `prevInvalid: true` を自動で立てる。「されてなかった」は `prevInvalid` に触れず、閉店チェック値を宵越し判定へ使える状態を維持する。
