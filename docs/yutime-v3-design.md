...
- `ramClear` は当日だけ有効。保存値は `cleared` のみで、ラムクリ傾向の生データとして残す。翌日のマップ表示は未確認に戻るが、日付付き記録自体は残る。
...
| `ytv3:premigrate` | 将来のスキーマ変更時に、旧 schema の raw データを退避する | 読み込み時、保存済み `version` が実装中の `SCHEMA_VERSION` と異なり、かつ `ytv3:premigrate` が未作成の場合。B1 では schema 1 から 2、B2 では schema 2 から 3、B3 では schema 3 から 4、B4 では schema 4 から 5、B5 では schema 5 から 6、Phase 4差し戻し1では schema 10 から 11、B10では schema 11 から 12、B20では schema 12 から 13、B13/B22では schema 13 から 14、B23では schema 14 から 15 への更新時に作成対象となる | 変更前の `ytv3:data` raw 文字列 |
...
- ラムクリ傾向分析: `dailyState` に蓄積した `ramClear: "cleared"` と閉店チェック回転数から、前日ヤメ回転数帯ごとのリセット率を見る。
...
- schema は 15 とする。`dailyState[machineId][date]` に `prevInvalid: bool` を追加し、`ramClear` は `cleared` / null に単純化する。
...
- マップの旧状態バッジ「リ」「不」「当」「据」は撤去する。ラムクリ記録がある当日だけ「ラ」マークを表示する。
- マップ上部に「朝チェック」トグルを置く。ON中に台番をタップすると、前日分無効チェックとラムクリチェックを持つミニ選択を開く。ラムクリをONにすると前日分無効も自動ONにする。
