...
## 85. S9 1R実質出玉の可視化・実測平均化

- schema 変更はない。保存項目は増やさず、表示と入力経路を足す。期待値の式も変えない。
- 期待値計算はエンジン内部で `netBallsPerWin`（1R実質出玉）を使っているが、判定画面のどこにも「いま使っている値」が出ていなかった。回転率には手入力欄・実測平均・参考値の3点セットがあるのに、1R実質出玉には無い。「判断の場に前提を置く」原則への違反なので、同じ3点セットを作る。
- **手入力欄**: 台詳細の期待値判定パネルに `evManualNetBalls`（1R実質出玉（手入力））を回転率の手入力の直後に置く。プレースホルダは自動決定の値と出典（例 `1,350玉（実測平均・n=4）`）。空欄なら自動決定に戻る。値は `renderMachineExpectation()` と `expectationPanelPresets()` の両方へ渡し、パネルに出ている判定と「この台を打つ」で記録される打ち始め期待値が食い違わないようにする。
- **採用順位**: `netBallsPerWinInfo(presetId, machine, manualInput)` に第3引数を足し、**パネルの手入力 → プリセット設定の手入力 → 実測平均 → ラウンド集計 → 理論値**とする。B52の順位に、その場の手入力を1段だけ上に積んだ形。ラウンド集計の段は既存データの値を動かさないために残す。
- **実測平均の出典をS8に合わせる**: 実測平均は「当選ごとの今回分（`actualBalls`）の合計 ÷ 当選件数」を第一とし、当選ごとの記録が無い旧セッションだけ `sessionActualBalls ÷ 当選件数` を使う。採用順位を `sessionActualBallsTotal()` と同じにして、実測出玉合計と実測平均の出典を1つにそろえた。S8で保留した `netBallsPerWinInfo()` の対応がこれにあたる。
- **参考1R出玉**: 台詳細のメトリクスに参考回転率の隣で出す（`参考1R出玉 1,350玉 / 実測平均・n=4`）。稼働中パネルは期待値セクションの「現在の実測回転率で再判定」の根拠行に、実測回転率の隣で出す。集計スコープは参考回転率と同じ台単位（`netBallsPerWinInfo` に machine を渡す）。
- **根拠行**: `使用回転率 17.0（手入力） / 使用1R実質出玉 990玉（実測平均・n=2） / 残り…` の順にする。従来 `payoutSource` が持っていた `純払い出し990玉/回（実測平均 n=2）、時短…` のうち、1R実質出玉の部分を `netBallsSource` へ切り出し、`payoutSource` は時短・遊タイムの玉/回転だけにした（同じ数字を1行に2回出さないため）。
- 表示の書式は `netBallsText()` に集約し、**小数を切り捨てない**（アグネスPEの理論値は `588玉` ではなく `587.5玉` と出す）。出典は `netBallsSourceText()` で「手入力」「実測平均・n=◯」「ラウンド集計・n=◯」「理論値」の4種。
- 検算: 手入力105 → `使用1R実質出玉 105玉（手入力）` で期待値がその値で再計算される。空欄に戻すと自動決定へ戻り、期待値も元の値に戻る。当選記録なし → `587.5玉（理論値）`。S8適用後、連チャンをまたぐセッション（1,380/1,420/1,380/1,220）でも実測平均は 5,400 ÷ 4 = **1,350玉** になる。
- 回帰: 旧データ6形をS8時点と実表示で突き合わせ、**期待値・判定・時給・機械割・1R平均はすべて不変**。差は根拠行の書式と理論値の桁（588→587.5）だけ。唯一値が動くのは `sessionActualBalls` が当選ごとの合計と食い違う旧セッションで、実測平均が 1,400玉 → 990玉（期待値 +10,089円 → +6,008円）になる。これはS9-1-2が指示した「今回分を出典にする」の帰結。
- 狭幅320/375/412pxでモーダル内のはみ出し・横スクロールが無いこと、稼働中パネルが縦にはみ出さないことを headless で確認した。

## 86. S10 1R実質出玉の実測平均を合計R数ベースに統一

- schema 変更はない。`netBallsPerWinInfo()` の実測平均の**分母だけ**を変える。手入力・ラウンド集計・理論値の経路は触らない。
- 「1R当たり」の一般的な意味は `獲得出玉の合計 ÷ 消化した合計R数`（6Rで540玉なら90玉）。S9の実測平均は当選件数で割っていたため、当り1回あたりの構成R数（4R/6R/10R）を無視していた。10R構成が多い台では過大、4R構成が多い台では過小に出る。戦果報告・大当たり登録の「1R当たり」（`sessionBallsPerRound`、S5で統一済み）は既に合計R数で割る定義で、S9の実測平均だけが取り残されていた。
- 実測平均を `Σ(今回分の獲得出玉) / Σ(消化R数)` にする。消化R数は `roundCountFromRoundType(roundTypeById(presetId, hit.roundTypeId))` で取り、`totalRoundsForPreset` と同じ1本の出所を使う（新しい算出は書かない）。
- **獲得出玉が未入力の当選は分子にも分母にも入れない**。R数だけ解決できて出玉が無い当選を分母に混ぜると平均が薄まるため。R種別が解決できずR数が取れない当選も同じ理由で両方から外す。
- 当選ごとの記録が無い旧データ（B90の `sessionActualBalls` だけを持つセッション）は `ヤメ入力の累計 ÷ そのセッションの合計R数`（`totalRoundsForPreset`）にする。S9のフォールバックを同じ分母へ移しただけ。
- `count` の意味を「当選件数」から「集計に使った合計R数」に変え、`countUnit: "rounds"` を添える。表示は `n=◯` ではなく **`◯R分`**（`実測平均・14R分`）。件数と読み違えないようにするため。ラウンド集計の `n=◯` は当選件数のままなので表記も変えていない。
- 検算: 6R・540玉 → **90玉**／10R 1,300玉＋4R 420玉 → 1,720÷14 ≒ **123玉**／10R（出玉未入力）＋4R 420玉 → **105玉**／当選1回・10R 1,300玉 → 従来1,300玉 → **130玉**。連チャンをまたぐ 1,380/1,420/1,380/1,220（4R/6R/4R/4R）は 5,400÷18 = **300玉**で、同じセッションの戦果報告「1R当たり300玉」と一致する。
- **未解決の単位不整合（要フォロー）**: `netBallsPerWinInfo()` の戻り値はそのまま期待値エンジンの `netBallsPerWin` に入るが、エンジンは `winBalls = expectedWins × netBallsPerWin`、つまり**当選あたりの玉数**として使う。S10で実測平均だけが「玉/R」になったため、実測平均が採用されるときだけ単位が食い違う。実測: アグネスPE・10R1,380玉＋4R600玉の台で `990玉/当選 → 141.4玉/R`、期待値 **+6,008円 → -2,437円**（判定 打てる → 打てない）。ずれの倍率は当選あたりの平均R数（この例では7R）そのもの。理論値（587.5）・ラウンド集計（756）・手入力の経路は当選あたりのままなので値は不変。**表示（参考1R出玉・使用1R実質出玉）は仕様どおり正しくなったが、期待値へ渡す前に平均R数を掛けて当選あたりへ戻すか、理論値側も玉/Rへ揃えるかの決着が別途必要**。

## 87. S11 1R実質出玉の単位を期待値エンジンとそろえる

- schema は 32 とする。保存項目は増やさないが、`presetSettings[*].netBallsPerWin` の**意味**が「当選あたりの玉数」から「1Rあたりの玉数（玉/R）」に変わるため、31以前のデータは移行時に平均R数で割って玉/Rへ直す。旧データは `ytv3:premigrate` に退避される。
- S10で実測平均だけが玉/Rになり、理論値・ラウンド集計・手入力は当選あたりのままだったため、同じ `netBallsPerWin` に2つの単位が混ざっていた。エンジンは `winBalls = expectedWins × netBallsPerWin` と当選あたりで受けるので、実測平均が採用されると期待値が平均R数ぶん過小に出ていた（実測 +6,008円 → -2,437円）。**方針B（全部を玉/Rへ揃え、エンジン側で平均R数を掛ける）**で解消する。
- エンジンのプリセット `spec` に `averageRoundsPerWin`（当選1回あたりの平均R数）を追加し、`winBalls` の算出を `expectedWins × (netBallsPerWin × averageRoundsPerWin) + 電サポ増減` に変える。掛け算は `netBallsPerWinTotal` として先にまとめ、`expectedWins × 旧netBallsPerWin` と浮動小数まで同じ値になるようにした。
- 平均R数はR構成の重みをそのまま使い、新しい推定は足さない。**大海5SP**: `standardPayout 1400 ÷ standardRounds 10 = 140玉/R`、`1400 ÷ 140 = 10R`。**アグネスPE**: R種別は4R/6R/10Rどれも `standardPayout 648 ÷ standardRounds 6 = 108玉/R`、`587.5 ÷ 108 = 5.44R`。`108 × (587.5/108) === 587.5`・`140 × 10 === 1400` が浮動小数でも厳密に成り立つことをテストで固定した。
- `defaults.netBallsPerWin` を玉/Rにする（agnes-pe 587.5 → **108**、umi-sp5 1400 → **140**）。定数も `DEFAULT_NET_BALLS_PER_WIN = 1400` を廃止し `DEFAULT_NET_BALLS_PER_ROUND = 140` にした（名前で単位が分かるようにするため）。
- `netBallsPerWinInfo()` の**ラウンド集計**も玉/Rにそろえる（R種別の公称出玉 ÷ そのR数）。アグネスPEは4R/6R/10Rいずれも108玉/Rになり、理論値と一致する。手入力（S9のパネル入力・プリセット設定）と実測平均（S10）は既に玉/Rなので変換不要。
- `yutime-calc.html` はエンジンを取り込んでいるので同じ変更を入れる。ページ側の変換は **アグネスPE: `(value) => value`（1R実質出玉の入力をそのまま渡す。従来の `587.5 × value / 108` は不要になった）**、**大海5SP: `(value) => value / averageRoundsPerWin`（入力欄は当選あたりの純払い出しのままなので割る）**。
- 表示の単位表記もそろえた。基準値チップ「純払い出し 587.5玉」→「1R実質出玉 108玉」、設定モーダルの「純払い出し量」→「1R実質出玉（玉/R）」、ヒントも「純増 ÷ 消化した合計R数」と書き直した。
- **受け入れ検算（代表点が動かないこと）**: アグネスPE・カウンター150・回転率17・1R実質100玉・等価・現金 → **+1,569円**（変換前と同じ）。カウンター0・回転率17・105玉 → **+310円**。記事v5の期待値表90セルも全一致（`tests/yutime-calc.test.js`）。大海5SP・434回転・回転率17・28玉・現金は holdSpins=0 で **−499円**、holdSpins=5 で **−231円** で、いずれもS11の前後で不変。仕様書の −454円 は B91（残保留込みの引き戻し）以前の値で、S11以前から再現しない。
- S10で報告した4パターンの検算（アグネスPE・カウンター150・回転率17・等価・現金）:

| 出典 | S10（単位不整合） | S11（玉/R統一） |
|---|---|---|
| 実測平均 141.4玉/R（10R1,380玉＋4R600玉） | -2,437円 | **+3,812円** |
| 実測平均 200玉/R（ヤメ累計2,800玉・14R） | -1,854円 | **+6,983円** |
| ラウンド集計（756玉/当選 → 108玉/R） | +3,679円 | **+2,002円** |
| 理論値（587.5玉/当選 → 108玉/R） | +2,002円 | **+2,002円（不変）** |

- 理論値が不変であることが変換の正しさの根拠。ラウンド集計は玉/Rにそろえた結果 理論値と一致し、実測平均は理論値108玉/Rに対する比（141.4/108＝1.31、200/108＝1.85）どおり単調に上がる。

## 88. S12 貯玉引出の店種別判定への修正とリザルト表示の整理

- schema 変更はない。転記用の `貯玉引出` が非パーソナル店でもパーソナル式を返していたバグの修正と、リザルト画面の表示改善2件。実収支・差玉の式は変更しない。
- 原因は `transferSummaryForSession()` が `store` を取っていながら `withdrawBalls` の算出で参照していなかったこと。B84で確定した「パーソナル／非パーソナルで式を分ける」ルールが、S4/§5の修正で貯玉引出から再び落ちた（同型の事故の2回目）。9/5の実戦で、非パーソナル店のセッションが開始持ち玉4,625個のまま表示され、実際のタップ投資合計2,375個と食い違って発覚した。実収支・差玉には影響しないが、この値は転記に使うため実害がある。
- 再発防止として、収支まわりの分岐を `usesPersonalBalanceFormula(store)` の1関数にまとめ、投資玉数そのものも `playerInvestedBalls(session, totals, store)` に集約した。`profitYenForSession()` と `withdrawBalls` は同じ関数を通す。条件式もパーソナル式そのものも他所へ書き戻さないことは、`tests/yutime-v3.test.js` で「ファイル全体で1箇所だけ」というアサートで固定した。
- **分岐の主軸は店の種別だけにする。タップ投資モードは条件に入れない。** パーソナル店には investments が空の記録（タップ投資導入前の記録方式。DSG高岡に開始持ち玉26,637個・11,462個の2件が実在）があり、`usesTapInvestmentMode()` を条件に混ぜると、その記録が非パーソナル式へ落ちて `持ち玉投入＋再プレ＝0玉` になってしまうため。`usesTapInvestmentMode()` 自体は回転率・通常消費玉の判定として引き続き使う。
- 定義は `貯玉引出 = プレイヤーが自分の玉から出した投資合計（現金は含めない）`。パーソナル店は `開始持ち玉 + 再プレ投資玉`（S4/§5で再プレを足した扱いは維持）、非パーソナル店は `持ち玉投入 + 再プレ投資玉`（B81由来の投資合計）。`貯玉預入`（終了玉＋残保留）と表裏になる。
- 検算: 非パーソナル店・125玉×19回 → **2,375個**（従来は開始持ち玉の4,625個）。パーソナル店・開始持ち玉2,500＋再プレイ500 → **3,000個（不変）**。パーソナル店・investments空・開始持ち玉26,637個／11,462個 → **26,637個／11,462個**。非パーソナル店でtapModeでない旧データは従来どおり `持ち玉投入＋再プレ`（回帰テストの合成データで 125個 → 375個）。
- **実収支への波及**: `playerInvestedBalls()` は実収支も通るため、パーソナル店でtapModeでない記録（investments空を含む）の実収支が、投資玉0扱いから `開始持ち玉＋再プレ` を引く形へ変わる。上記2件は開始持ち玉がそのまま投資玉として引かれる。B84の意図（パーソナル店は開始持ち玉が自分の玉）に沿った修正で、従来はこれらのセッションが台上の全玉を利益として数えていた。非パーソナル店の実収支は不変。
- B-1: 「想定と実測のズレ」の行ラベルを `期待値との差` → `開始期待値との差` にした。何と何を比べている行かを、行だけで読めるようにするため。差の算出（`実収支 − 開始期待値`。実測が上回れば正）と直下の注記行は変更しない。
