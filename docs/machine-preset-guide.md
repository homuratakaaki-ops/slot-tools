...
- `tagIds` と `tagLabels` は同一 index のペアとして保存・正規化する。ラベル未取得の位置には空文字を保持し、配列長は常に一致させる。
- 過去の圧縮バグで対応が失われたデータは修復不能のため、ラベル空として扱う。表示は3段フォールバックの次段に進む。
...
8. **カウント・ポイント系のクリア契機を先に定義する**: カウント・ポイント系の表示は、系統ごとに「何を契機にクリアされるか」を定義してから実装する。周期制・ポイント制の機種では、新機種プリセット導入時の必須確認項目とする。
