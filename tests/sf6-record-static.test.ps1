...
Assert-Fixed $html "const VERSION='3'" 'version 3 is missing'
...
Assert-Contains $html "cashKinds" 'cash chips are missing'
Assert-Contains $html "cashBtn" 'cash record button is missing'
Assert-Contains $html "collectMedals" 'collect medals field is missing'
Assert-Contains $html "collectBtn" 'collect record button is missing'
Assert-Contains $html "type:'cash'" 'cash record type is missing'
Assert-Contains $html "type:'collect'" 'collect record type is missing'
...
Assert-NotContains $html "rankKinds|selectedRank|const RANKS" 'rank input should be removed'
Assert-NotContains $html "rank:selectedRank" 'new FB records should not save rank'
...
Assert-Fixed $schema "const SCHEMA_VERSION='3'" 'schema version 3 is missing'
