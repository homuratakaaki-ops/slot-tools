(function(){
  ...
  // テンプレ表記（tpl* の列）は、なな様完成版テンプレの原文どおり。
  // 半角カナ・全角スペースの詰め物・絵文字はテンプレ出力専用で、UIには持ち込まない（§9-64）。

  // [id, 表示名, サブラベル, rank(確定設定/0=なし), カード略号, 示唆系統, テンプレ表記]
  const SCREENS=[
    ['sakuta','梓川咲太','デフォルト',0,'咲',null,'デフォルト'],
    ['tomoe','古賀朋絵','偶数設定示唆',0,'朋','even','🩷古賀朋絵(偶数)'],
  ...
  // アオハルチャンス（報酬獲得ゾーン）[id, 表示名, サブラベル, テンプレ表記]
  const AOHAL=[
    ['cherry','チェリー','成功30〜40%','ﾁｪﾘｰ'],
    ['chance','チャンス目','成功50〜60%','ﾁｬﾝｽ目']
  ];
  // 咲太クエスト（単カウント）[id, 表示名, サブラベル, rank, カード略号, テンプレ表記]
  const QUEST=[
    ['suika','スイカ','',0,'ス','スイカ'],
    ['chance','チャンス目','',0,'チ','ﾁｬﾝｽ目']
  ];
  // 不可思議モード（単カウント・サブは当該モードの効果）
  // [id, 表示名, サブラベル, rank, カード略号, テンプレ表記, テンプレ効果行]
  const FUSHIGI=[
    ['tomoe','古賀朋絵','次回規定ポイントが100pt',0,'朋','💚古賀朋絵　','↪︎次回100pt'],
    ['nodoka','豊浜のどか','アオハルCHANCE報酬優遇',0,'の','🩵豊浜のどか','↪︎ｱｵﾊﾙﾁｬﾝｽ報酬↑'],
    ['rio','双葉理央','常にアオハルCHANCE高確率',0,'理','❤️双葉理央　','↪︎ｱｵﾊﾙﾁｬﾝｽ高確'],
    ['mai','桜島麻衣','規定ポイント500pt以内',0,'麻','💙桜島麻衣　','↪︎当該500pt以内'],
    ['kaede','梓川かえで','次回CZに梓川かえで出現',0,'か','🧡梓川かえで','↪︎次回CZ'],
    ['shoko','牧之原翔子','次回CZに牧之原翔子出現',0,'翔','💜牧之原翔子','↪︎次回CZ']
  ];
  // CZエピソードチャンス 最終ゲームの特殊抽選（キャラ別・成立役別）
  // [id, 表示名, [行id, UI表示名, テンプレ表記], テンプレ見出し]
  const CZ_CHARS=[
    ['tomoe','古賀朋絵',[['t1','リプレイ・チェリー','ﾘﾌﾟ/ﾁｪﾘｰ'],['t2','ベル','ﾍﾞﾙ'],['t3','チャンス目','ﾁｬﾝｽ目']],'💚古賀朋絵'],
    ['nodoka','豊浜のどか',[['n1','リプレイ','ﾘﾌﾟ'],['n2','ベル・スイカ・チェリー','ﾍﾞﾙ/ｽｲｶ/ﾁｪﾘｰ'],['n3','チャンス目','ﾁｬﾝｽ目']],'🩵豊浜のどか'],
    ['rio','双葉理央',[['r1','リプレイ・スイカ','ﾘﾌﾟ/ｽｲｶ'],['r2','ベル','ﾍﾞﾙ'],['r3','チャンス目','ﾁｬﾝｽ目']],'❤️双葉理央'],
    ['mai','桜島麻衣',[['m1','リプレイ','ﾘﾌﾟ'],['m2','ベル','ﾍﾞﾙ'],['m3','チャンス目','ﾁｬﾝｽ目']],'💙桜島麻衣'],
    ['kaede','梓川かえで',[['k1','リプレイ','ﾘﾌﾟ'],['k2','ベル','ﾍﾞﾙ']],'🧡梓川かえで'],
    ['shoko','牧之原翔子',[['s1','リプレイ','ﾘﾌﾟ'],['s2','ベル','ﾍﾞﾙ']],'💜牧之原翔子']
  ];
  ...
  // なな様完成版テンプレの行構成・表記・絵文字に一致させる。原文は記入枠として
  // 全行が並ぶ形式のため、未記録でも全行を出す。セクション間に空行は入れない。
  // 初当りは見出し行（通常 xG / 初当り n回）が担うため、原文どおり専用セクションは持たない。
  function tplText(ctx){
    const S=ctx.S,g=S.games,first=n(S.counts,'first');
    const L=['設定判別メモ｜L青春ブタ野郎はバニーガール先輩の夢を見ない',
      `通常 ${g||0}G / 初当り${first}回`,'_______','',
      '■規定ﾎﾟｲﾝﾄ当選率'];
    POINTS.forEach(p=>L.push(`${p[1]}▶︎ ${ndText(S,'points',p[0])}`));
    L.push('↪︎400pt前兆移行率1.0〜2.8％');
    L.push('■通常時ｱｵﾊﾙﾁｬﾝｽ(報酬獲得ｿﾞｰﾝ)',
      AOHAL.map(v=>`${v[3]}▶︎ ${ndText(S,'aohal',v[0])}`).join(' ・'),
      '↪︎30〜40％・50〜60％');
    L.push('■咲太ｸｴｽﾄ');
    QUEST.forEach(c=>L.push(`${c[5]}▶︎ ${countLine(n(S.quest,c[0]))}`));
    L.push('■不可思議ﾓｰﾄﾞ');
    FUSHIGI.forEach(c=>{
      L.push(`${c[5]}▶︎ ${countLine(n(S.fushigi,c[0]))}`);
      L.push(c[6]);
    });
    L.push('■CZｴﾋﾟｿｰﾄﾞﾁｬﾝｽ最終ｹﾞｰﾑ特殊抽選');
    CZ_CHARS.forEach(c=>{
      L.push(c[3]);
      c[2].forEach(r=>L.push(`${r[2]}▶︎ ${ndText(S,'czr',r[0])}`));
    });
    L.push('■ST終了画面');
    SCREENS.forEach(c=>L.push(`${c[6]}▶︎ ${countLine(n(S.screens,c[0]))}`));
