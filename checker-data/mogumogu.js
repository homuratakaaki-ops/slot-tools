(function(){
  ...
  // モグ玄・騎乗モグ玄は出典が「SPモード1つ以上 or 設定2以上濃厚」の形で、
  // SPモードへ移行したかどうかで意味が変わる。条件付きのまま確定演出に数えると
  // 設定の確定を過大に見せるため、SP非移行を確認できた場合とそうでない場合で
  // 行を分ける。確定演出に数えるのは SP非移行 の行だけ。
  const AT_END=[
    ['kerai','家来','奇数設定示唆',0,'家'],
    ['taihou','大砲家来','偶数設定示唆',0,'砲'],
    ['ninmogu','忍モグ＆くモ一','高設定示唆(弱)',0,'忍'],
    ['hime','モグ姫','高設定示唆(弱)',0,'姫'],
    ['gen','モグ玄(SP非移行)','設定2以上確定演出',2,'玄2'],
    ['genSp','モグ玄(SP移行あり・不明)','SPモード移行か設定2以上',0,'玄?'],
    ['kijou','騎乗モグ玄(SP非移行)','設定3以上確定演出',3,'騎3'],
    ['kijouSp','騎乗モグ玄(SP移行あり・不明)','SPモード移行か設定3以上',0,'騎?'],
    ['teitoku','モグー提督','設定4以上確定演出',4,'提'],
    ['zenin','全員集合','設定6確定演出',6,'全'],
    // 出典の一覧は上の8種（モグ玄・騎乗モグ玄は上で2行ずつに分けている）。
    // どれにも当てはまらない画面が出たときの受け皿を置き、割合の分母を閉じる
    // （出典に無い画面を作らないため、示唆は付けない）。
  ...
  function pageShisa(ctx){
    ...
    <div class="hint">SPモードに入らなかった時だけ確定演出の行へ。モグ玄と騎乗モグ玄は、出典が「SPモード1つ以上 or 設定2以上」「SPモード2つ以上 or 設定3以上」の形になっていて、SPモードへの移行の有無で意味が変わります。AT後の挙動まで確認できて、SPモードへ移行しなかった場合だけ「(SP非移行)」の行に記録してください（騎乗はSPモード2つ以上への非移行が条件です）。移行した場合や確認できなかった場合は「(SP移行あり・不明)」の行に記録します。確定演出として数えるのは「(SP非移行)」の行だけです。</div>
