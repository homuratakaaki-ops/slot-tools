(function(){
  ...
  // 通常時の小役確率（出典: ちょんぼりすた様、2026/9/25 再取得）。本機の設定は1・2・5・6の4段階。
  // ユニメモで基本カウントされるのは プラム・チェリー合算・スイカ合成 の3つ。
  // スイカA・B と チェリーA〜C は、ユニメモにログインして特定回数のタコゲームを
  // 発生させ、条件を解放するとカウントできるようになる（出典の注記どおり）。
  ...
  // チェリーA（設1・2・5・6:1/184.6）は設定差が無いので行を置かない。
  // 表面上チェリーAとBの見分けはつかず、チェリー合算は
  // 「チェリーA・Bの成立回数 ＋ チェリーCの入賞回数」で算出されている。
  const KOYAKU=[
    ['plum','プラム','設1:1/9.8⇔設6:1/9.0','9.8'],
    ['cherry','チェリー合算','設1:1/22.9⇔設6:1/20.1','22.9'],
    ['melon','スイカ合成','設1:1/60.0⇔設6:1/50.0','60.0'],
    ['cherryB','チェリーB','設1:1/136.5⇔設6:1/109.2','136.5'],
    ['cherryC','チェリーC','設1:1/32.3⇔設6:1/28.5','32.3'],
    ['melonA','スイカA','設1:1/66.6⇔設6:1/55.5','66.6'],
    ['melonB','スイカB','設1:1/601.2⇔設6:1/504.1','601.2']
  ];
  const DEF={
    games:0,
    gameSrc:'unimemo',
  ...
  <section class="sec">
    <div class="sec-h">小役確率（ユニメモ）<span class="sub">入力 ${appFilled(S).length}/${KOYAKU.length}</span></div>
    <style>
    ...
    <div class="hint">下4つはユニメモの条件解放が必要です。チェリーB・C とスイカA・B は、ユニメモで条件を解放すると確認できます。ユニメモにログインし、特定回数のタコゲームを発生させると、これらの内訳がカウントされるようになります。解放していない場合は空欄のままで構いません。チェリーA（設1・2・5・6とも1/184.6）は設定差が無いため行を設けていません。表面上チェリーAとBの見分けはつかず、チェリー合算は「チェリーA・Bの成立回数＋チェリーCの入賞回数」で算出されています。</div>
  </section>
  <section class="sec">
  ...
  function pageSuggest(ctx){
    ...
    <div class="hint">ユニメモと役割を分けています。ユニメモはこのツールでは通常回転数（分母）と小役確率の入力に使います。BB終了画面は出るたびにこちらで記録してください。同じ遊技分の分母と示唆が揃い、初当り確率と終了画面を突き合わせて見られます。</div>
  </section>
  <section class="sec">
