(function(){
  ...
  <section class="sec">
    ...
    <div class="hint">帯の表示で見分けられます。確定CZはCZ突入時に「THE ENDモード」の帯が表示されるので見分けられます。出現率が低いため1/x表示は行わず、引けた場合の判別材料として扱ってください。</div>
    <div class="hint">ダイトモの確率の分母をそのまま入れます。共通ベルと強チャンス目Bは、ダイトモ（大都技研の公式アプリ）の遊技履歴に表示される確率の数値をそのまま入力してください。小役はAT中も成立するため、アプリが集計した確率をそのまま使うのが正確です。</div>
  </section>`;
  }
  ...
  <section class="sec">
    ...
    <div class="hint">この2項目の記録は任意です。通常時スイカのシューティングチャージ当選（設1:40.2%⇔設6:55.1%）と強チェリーのCZ当選にも設定差あり。counting負荷が高いため本ツールでは任意。共有テキストへ手書き追記推奨。</div>
  </section>`;
  }
