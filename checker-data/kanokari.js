(function(){
  ...
  // ED_VOICE だけ6列目に「詳細カード用の略記」を持つ（画面の行は出典どおりのまま）。
  ...
  // 6列目は詳細カード用の略記。詳細カードの行は幅420px・最小18px・省略記号なしで描かれ、
  // 全角23.3字を超えると右列（x=560）に掛かるため、長いセリフだけ「…」で短縮する。
  // 画面（示唆タブ）とテンプレ出力は2列目の出典どおりの表記を使い、短縮しない。
  const ED_VOICE=[
    ['v01','麻美「アガる～↑」','示唆内容は非公表',0,'麻1','麻美「アガる～↑」'],
    ['v02','瑠夏「彼女入りまーす」','示唆内容は非公表',0,'瑠1','瑠夏「彼女入りまーす」'],
    ['v03','墨「ふん、ふん、！！」','示唆内容は非公表',0,'墨1','墨「ふん、ふん、！！」'],
    ['v04','水原「今は”恋人”。遠慮しない」','示唆内容は非公表',0,'水1','水原「今は”恋人”…」'],
    ['v05','麻美「あれ～？嫉妬させちゃった？」','示唆内容は非公表',0,'麻2','麻美「あれ～？嫉妬させちゃった？」'],
    ['v06','瑠夏「私が一番…好きだもん…っ」','示唆内容は非公表',0,'瑠2','瑠夏「私が一番…」'],
    ['v07','墨「私…っいるか…っ」','示唆内容は非公表',0,'墨2','墨「私…っいるか…っ」'],
    ['v08','麻美「もう恋なんてしないって決めてるんだから！」','示唆内容は非公表',0,'麻3','麻美「もう恋なんて…」'],
    ['v09','瑠夏「なんだか少し、お酒の味…」','示唆内容は非公表',0,'瑠3','瑠夏「お酒の味…」'],
    ['v10','墨「今日は私がお饗しする番…」','示唆内容は非公表',0,'墨3','墨「私がお饗しする番…」'],
    ['v11','水原「私……どんなカオ…してたかな……」','示唆内容は非公表(公式が『特に注目』と記載)',0,'水2','水原「どんなカオ…」']
  ];
  ...
  // 詳細カードだけ6列目の略記を使う（無い行は2列目のまま）。§9-85 の省略をデータ側で行う形。
  function detailItemsShort(arr,state){return arr.map(c=>detailItem(c[5]||c[1],n(state,c[0]),c[3]>0));}
  ...
  function detail(ctx){
    ...
    return [
      ...
      // ボイスは行幅の都合で6列目の略記を使う（画面の行は出典どおりのまま）。
      {title:'エンディング中のボイス',items:detailItemsShort(ED_VOICE,S.voice),denominator:total(ED_VOICE,S.voice)}
    ];
  }
